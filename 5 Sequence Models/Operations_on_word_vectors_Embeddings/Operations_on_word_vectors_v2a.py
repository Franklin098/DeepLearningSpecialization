...
def equalize(pair, bias_axis, word_to_vec_map):
    ...
    corrected_e_w1B = np.sqrt(np.abs(1-np.sum(np.dot(mu_orth,mu_orth)))) * (e_w1B-mu_B)/(np.linalg.norm(e_w1-mu_orth-mu_B))
    corrected_e_w2B = np.sqrt(np.abs(1-np.sum(np.dot(mu_orth,mu_orth)))) * (e_w2B-mu_B)/(np.linalg.norm(e_w2-mu_orth-mu_B))
